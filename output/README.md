...
当前生成的 `wpa_mini` 大小为 `493520` 字节，`wpa_mini.run` 大小为 `342676` 字节。`wpa_mini` 是单一可执行文件，已经把精简 STA 连接引擎链接进程序内部，不需要额外交付 `wpa_cli` 或外部 `wpa_supplicant`。
...
- 确保 `/etc_rw/udhcpd.conf` 与当前 `br0` 网段一致，并启动 `udhcpd`，让 USB/RNDIS 和热点客户端能自动获取 IP、网关和 DNS。
